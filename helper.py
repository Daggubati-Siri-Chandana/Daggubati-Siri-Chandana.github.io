...
from BCA.generate_structure_tensor_lab import compute_structure_tensor
from BCA.data_extract import reconstruct_chart
from BCA.Retrieve_Text import remove_text, remove_legend
from BCA.Graph_Obj_Seg import segment
from BCA.Chart_Classification.model_loader import classifyImage
...
def image_read(filename,xml_file):
    image_name = os.path.basename(filename).split(".png")[0]
    path = os.path.dirname(filename)+'/'
    os.rename(filename,path+"chart.png")
    os.rename(xml_file,path+"chart.xml")
    image_name='chart'
    filename = path+"chart.png"
    xml_file = path+"chart.xml"
